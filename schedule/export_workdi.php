...
// $display_type   = $_SESSION['sch_filters']['display_type']; // ไม่ได้ใช้แล้ว แต่อาจจะเก็บไว้ได้
...
for ($d = 1; $d <= $days_in_month; $d++) {
    ...
    echo "<th style=\"$bg_color_inline color: #000000; text-align: center; vertical-align:top; width: 80px;\">{$d}<br><small>{$day_of_week}</small></th>";
}
echo '</tr></thead><tbody>';
        ...
        for ($d = 1; $d <= $days_in_month; $d++) {
            ...
            if (count($daily_schedules) > 0) {
                ...
                foreach ($daily_schedules as $sch) {
                    ...
                    // ดึงค่า workdi_code จาก API มาแสดงแทน
                    // ถ้าค่าว่างหรือไม่พบ ให้แสดงเป็น FREE
                    $workdi_code = !empty($sch['workdi_code']) ? htmlspecialchars($sch['workdi_code']) : 'FREE'; 
                    
                    // นำมาแสดงเลยโดยไม่สนใจสถานะ
                    $contents[] = "<strong>{$workdi_code}</strong>";
                }
            } else {
                // กรณีไม่มีข้อมูลกะในวันนั้น ให้แสดงเป็น FREE แทนช่องว่าง
                $contents[] = "<strong>FREE</strong>";
            }
